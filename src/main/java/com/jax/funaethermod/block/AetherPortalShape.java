...
public class AetherPortalShape {
    ...
    (state, level, pos) -> state.is(ModBlocks.AETHER_PORTAL_FRAME.get());
    ...
    private int getDistanceUntilTop(BlockPos.MutableBlockPos mutablePos) {
        for (int i = 0; i < 21; ++i) {
            ...
            for (int j = 0; j < this.width; ++j) {
                ...
                if (blockState.is(ModBlocks.AETHER_PORTAL.get())) {
    ...
    private static boolean isEmpty(BlockState state) {
        return state.isAir() || state.is(Blocks.WATER) || state.is(ModBlocks.AETHER_PORTAL.get());
    }
    ...
    public void createPortalBlocks() {
        BlockState blockState = ModBlocks.AETHER_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, this.axis);
